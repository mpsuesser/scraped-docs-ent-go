---
url: https://entgo.io/docs/interceptors
title: "Interceptors"
description: ""
...
It is recommended to add the [`schema/snapshot`](feature-flags.md#auto-solve-merge-conflicts) feature-flag along with the `intercept` flag to enhance the development experience, for example:
...
> **Info:**
> 
> You should notice that similar to [schema hooks](hooks.md#hooks-registration), if you use the **`Interceptors`** option in your schema, you **MUST** add the following import in the main package, because a circular import is possible between the schema package and the generated ent package:
> 
> ```markdown
...
In some cases, there is a need to intercept [graph traversals](traversals.md) and modify their builders before continuing to the nodes returned by the query. For example, in the query below, we want to ensure that only `active` users are traversed in **any** graph traversals in the system:
...
The soft delete pattern is a common use-case for interceptors and hooks. The example below demonstrates how to add such functionality to all schemas in the project using [`ent.Mixin`](schema-mixin.md):
