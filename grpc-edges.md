---
url: https://entgo.io/docs/grpc-edges
title: "Grpc Edges"
description: ""
...
- We created a one-to-many relationship where a `Category` has a single `admin`, and a `User` can administer multiple categories.
...
- A new field `administered` was added to the `User` message definition. It is a `repeated` field, corresponding to the fact that we did not mark the edge as `Unique` in this direction. It's field number is `5`, corresponding to the `entproto.Field` annotation on the edge.
