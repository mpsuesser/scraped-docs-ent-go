---
url: https://entgo.io/docs/schema-indexes
...
In the example above, we have a `City` with many `Street` s, and we want to set the street name to be unique under each city.
...
The Ent schema supports defining indexes on fields and edges (foreign-keys), but there is no API for defining index parts as expressions, such as function calls. If you are using [Atlas](https://atlasgo.io/docs) for managing schema migrations, you can define functional indexes as described in [this guide](migration/functional-indexes.md).
