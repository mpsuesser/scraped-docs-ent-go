...
Ent supports [upsert](https://en.wikipedia.org/wiki/Merge_\(SQL\)) records using the [`sql/upsert`](feature-flags.md#upsert) feature-flag.
...
Select partial objects and partial associations. Get all pets and their owners, but select and fill only the `ID` and `Name` fields.
...
In some projects, the "delete many" operation is not allowed and is blocked using hooks. However, there is still a need to delete a single entity by its ID while ensuring it meets a specific condition. In this case, you can use the `Where` option as follows:
