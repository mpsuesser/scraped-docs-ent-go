---
url: https://entgo.io/docs/code-gen
title: "Code Gen"
description: ""
...
After adding a few [fields](schema-fields.md) and [edges](schema-edges.md), you want to generate the assets for working with your entities. Run `ent generate` from the root directory of the project, or use `go generate`:
...
One of the options for achieving this is asking `go generate` to use the version mentioned in the `go.mod` file when running `ent`. If your project does not use [Go modules](https://github.com/golang/go/wiki/Modules#quick-start), setup one as follows:
...
In order to extend the generated client and builders under the `ent` package, and inject them external dependencies as struct fields, use the `entc.Dependency` option in your [`ent/entc.go`](#use-entc-as-a-package) file:
...
For more information, please see the [features-flags page](feature-flags.md).
